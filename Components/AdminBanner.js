...
export default function AdminBanner() {
  ...
  const removeProduct = () => {
    axios
      .delete(`${APIUrls.banner}/${currentID}`)
      .then((res) => {
        alert.show("Deleted", {
          type: "info",
        });
		...
		c-0.6,0-1-0.4-1-1V28c0-0.6,0.4-1,1-1h2c0.6,0,1,0.4,1,1V42z"
                    ...
                    <td className="th-center">
                      <input
                        type="checkbox"
                        checked={d.isActive}
                        onChange={(e) => handleActive(e, d._id)}
                      ></input>
                    </td>
                  </tr>
                );
              ...
              <tr>
                <td colSpan="7" className="th-center">
                  No data
                </td>
              </tr>
            )}
                ...
                >
                  {itemList && itemList.length > 0
                    ? itemList.map((d, i) => (
                        <option
                          key={i}
                          value={`/product/${d.category}/${d._id}`}
                        >
                          {d.title}
                        </option>
                      ))
