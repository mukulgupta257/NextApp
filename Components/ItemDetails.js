...
export default function ItemDetails(props) {
  ...
  return (
    <>
      <Head>
        <title>
          {product.title ? `${product.title} | Vedayuu` : "Loading..."}
        </title>
      </Head>
      <div className="item-details-wrapper container">
        <div className="row">
          <div className="col-12 col-md-6">
            {displayImage ? (
              <Image src={displayImage} width={640} height={640}></Image>
            ) : (
            ...
            <div className="sub-images d-flex flex-wrap">
              {thumbnailImages
                ? thumbnailImages.map((value, key) => {
                    return (
                      <div className="col-4 col-lg-3 product" key={key}>
                        <Image
                          src={value}
                          width={144}
                          height={144}
          ...
          <div className="col-12 col-md-6">
            <div className="item-description">
              <h1>{product.title}</h1>
              <h3>
                {`Rs. ${variantSalePrice ? variantSalePrice : ""}`}{" "}
                <strike>{variantPrice ? variantPrice : ""}</strike>
              </h3>
              <h4>
                <span>SAVE ({variantPrice - variantSalePrice}) </span>
              </h4>
              <div className="input-wrapper">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setDisplayImage(
                      e.target.value.split("|")[3] == "null"
                        ? null
                        : e.target.value.split("|")[3]
                    );
                    setThumbnailImages(
                      e.target.value.split("|")[4] == "null"
                        ? []
                        : JSON.parse(e.target.value.split("|")[4])
                    );
                ...
                >
                  {product.variants
                    ? product.variants.map((data, key) => {
                        return (
                          <option
                            key={key}
                            selected={
                              variantColor == data.color ? "selected" : ""
                            }
                            value={`${data.price}|${data.color}|${data.qty}|${
                              data.displayImage ? data.displayImage : null
                            }|${
                              data.thumbnailImages
                                ? JSON.stringify(data.thumbnailImages)
                                : null
              ...
              <div className="input-wrapper">
                ...
                <select
                  ...
                  id="qty"
                  onChange={(e) => {
                    setSelectedVariantQty(e.target.value);
                  }}
                >
                  {qtyArray
                    ? qtyArray.map((value, index) => {
                        return (
                          <option
                            key={index}
                            selected={
                              selectedVariantQty == index + 1 ? "selected" : ""
                            }
                            value={index + 1}
                          >
                            {index + 1}
              ...
              <p
                ...
                dangerouslySetInnerHTML={{ __html: variantDesc }}
              ></p>
